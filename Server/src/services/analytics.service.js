/**
 * Servicio de analítica y métricas del negocio.
 * Extraído de superadmin.service.js para separar analytics de gestión de negocios.
 */
...
// 1. Métricas globales (finanzas, usuarios, citas, top servicios)
export const getGlobalMetrics = async (businessId) => {
  ...
  const paymentStats = await paymentRepository.aggregateFinancialMetrics(matchPayment);
  ...
  const userStats = await userRepository.aggregate([
    ...userStatsStages,
    {
      $group: {
        _id: "$role",
  ...
  const appointmentStats = await appointmentRepository.aggregate([
  ...
  const topServices = await appointmentRepository.aggregate([
...
// 2. Análisis avanzado (concurrencia horaria, días concurridos, tendencias y feriados)
export const getAdvancedAnalytics = async (businessId) => {
  ...
  // A. Concurrencia horaria
  const hourlyConcurrency = await appointmentRepository.aggregate([
    { $match: { status: { $ne: "cancelled" }, ...matchStage } },
    { $group: { _id: "$startTime", totalBookings: { $sum: 1 } } },
    { $sort: { totalBookings: -1 } },
  ]);

  // B. Días de la semana
  const dayOfWeekConcurrency = await appointmentRepository.aggregate([
    { $match: { status: { $ne: "cancelled" }, ...matchStage } },
  ...
  const monthlyTrends = await appointmentRepository.aggregate([
    { $match: { status: { $ne: "cancelled" }, ...matchStage } },
    { $group: { _id: { $month: "$date" }, totalBookings: { $sum: 1 } } },
    { $sort: { _id: 1 } },
  ]);
  ...
  // D. Tendencia semanal
  const weeklyTrends = await appointmentRepository.aggregate([
    { $match: { status: { $ne: "cancelled" }, ...matchStage } },
    { $group: { _id: { $week: "$date" }, totalBookings: { $sum: 1 } } },
    { $sort: { _id: 1 } },
  ]);
  ...
  // E. Correlación con días feriados
  ...
  if (holidays.length > 0) {
    ...
    const dailyBookings = await appointmentRepository.aggregate([
      { $match: { status: { $ne: "cancelled" }, ...matchStage } },
      {
        $group: {
          _id: {
    ...
    dailyBookings.forEach((day) => {
      ...
      if (isHoliday) { holidaySum += day.count; holidayCount++; }
      else if (isEve) { eveSum += day.count; eveCount++; }
      else { normalSum += day.count; normalCount++; }
    });

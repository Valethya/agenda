...
export const AVAILABILITY_TENANTIZATION_VERSION = "1.1.0";
...
export const AVAILABILITY_TENANTIZATION_LOCK_KEY = "availability-tenantization-6.2.3";
...
const parseMongoUriTarget = (mongoUri) => {
  ...
  if (slashIndex === -1) throw new Error("MONGO_URI debe codificar explícitamente la base de datos");
...
export const validateAvailabilityTenantizationRuntime = ({
  ...
  if (deploymentIndicator) {
    throw new Error("La migración 6.2.3 debe ejecutarse desde un operador aislado, no dentro de una plataforma de despliegue");
  }
  ...
  if (LOCAL_ENVIRONMENTS.has(effectiveEnvironment)) {
    ...
    if (!localTarget) {
      throw new Error("development/test sólo permiten MongoDB local; un destino externo exige staging/production y autorización explícita");
    }
    return { effectiveEnvironment, targetKind: "local", effectiveCodeSha: null };
  }

  if (!REMOTE_ENVIRONMENTS.has(effectiveEnvironment)) {
    throw new Error("Entorno operativo no reconocido");
  }
  if (localTarget) {
  ...
  if (allowExternalTarget !== AVAILABILITY_EXTERNAL_TARGET_CONFIRMATION) {
    throw new Error(`El destino externo exige --allow-external-target=${AVAILABILITY_EXTERNAL_TARGET_CONFIRMATION}`);
  }
  if (mode === "apply" && maintenanceWindow !== AVAILABILITY_MAINTENANCE_CONFIRMATION) {
    throw new Error(`Apply externo exige --maintenance-window=${AVAILABILITY_MAINTENANCE_CONFIRMATION}`);
  }
  const expected = validateCodeSha(expectedCodeSha);
  const { codeSha } = resolveEffectiveCodeSha({
    railwayGitCommitSha: processEnvironment?.RAILWAY_GIT_COMMIT_SHA,
    githubSha: processEnvironment?.GITHUB_SHA,
    explicitCodeSha: processEnvironment?.AVAILABILITY_TENANTIZATION_CODE_SHA,
  });
  if (!codeSha) {
    throw new Error("El destino externo exige provenance del SHA efectivo mediante AVAILABILITY_TENANTIZATION_CODE_SHA o proveedor soportado");
  }
  if (codeSha !== expected) {
...
const resolvedDocuments = (classification) => [
  ...
  ...classification.deterministic.map(({ document, inferredBusiness }) => ({ document, business: inferredBusiness })),
];
...
const activeAppointmentConflicts = (appointments) => {
  ...
  return { invalid, duplicateKeys: [...counts.values()].filter((count) => count > 1).length };
};
...
const collectionNames = async (db, session) => new Set(
  (await db.listCollections({}, { nameOnly: true, session }).toArray()).map((item) => item.name),
);
const readCollection = async (db, names, name, projection, session) => {
  if (!names.has(name)) return [];
  return db.collection(name).find({}, { projection, session }).toArray();
};
...
export const readAvailabilityTenantizationSnapshot = async (db, { session } = {}) => {
  const names = await collectionNames(db, session);
  const [shifts, blocks, memberships, appointments, shiftIndexes, blockIndexes, appointmentIndexes] = await Promise.all([
    readCollection(db, names, "shifts", { _id: 1, business: 1, worker: 1, dayOfWeek: 1 }, session),
    readCollection(db, names, "blocks", { _id: 1, business: 1, worker: 1, date: 1 }, session),
    readCollection(db, names, "memberships", { _id: 1, user: 1, business: 1, role: 1, isActive: 1 }, session),
    readCollection(db, names, "appointments", { _id: 1, business: 1, worker: 1, date: 1, startTime: 1, status: 1 }, session),
...
export const ensureAvailabilityTenantizationLockCollection = async (db) => {
  const exists = await db.listCollections({ name: AVAILABILITY_TENANTIZATION_LOCK_COLLECTION }, { nameOnly: true }).hasNext();
...
export const acquireAvailabilityTenantizationLock = async (
  ...
  if (!lock || !Number.isInteger(lock.fencingToken)) throw new AvailabilityTenantizationLockActiveError();
...
export const renewAvailabilityTenantizationLock = async (
  ...
  const result = await db.collection(AVAILABILITY_TENANTIZATION_LOCK_COLLECTION).updateOne(
    ...
    { $set: { leaseUntil: addMs(now, leaseMs), updatedAt: new Date(now) } },
  );
  if (result.modifiedCount !== 1) throw new AvailabilityTenantizationLockLostError();
  return true;
};

export const assertAvailabilityTenantizationLockOwnership = async (db, lock, { now = new Date() } = {}) => {
...
const uniqueActiveWorkerBusinesses = (memberships) => {
  ...
  for (const membership of memberships) {
    if (membership?.role === "worker" && membership?.isActive === true && isObjectId(membership.business)) {
...
const revalidateAssignmentMembership = async (db, assignment, session) => {
  ...
  if (businesses.length !== 1 || objectIdKey(businesses[0]) !== objectIdKey(assignment.inferredBusiness)) {
    throw new Error("Backfill abortado: Membership cambió desde el plan y la inferencia ya no es determinística");
  }
};

const applyBackfill = async (db, plan, session, mutationCheckpoint = async () => {}) => {
  const writeAssignments = async (collectionName, assignments) => {
    for (const assignment of assignments) {
      await revalidateAssignmentMembership(db, assignment, session);
      await mutationCheckpoint(`before-backfill:${collectionName}`, { db, session, assignment });
...
const ensureDesiredIndex = async (db, collectionName, spec) => {
  ...
  if (state.conflicts.length) throw new Error(`Índice destino conflictivo en ${collectionName}; no se modificará automáticamente`);
  if (!state.present) {
...
const dropObsoleteIndexes = async (db, collectionName, spec) => {
  ...
  for (const name of names) {
    await db.collection(collectionName).dropIndex(name, { maxTimeMS: AVAILABILITY_TENANTIZATION_DDL_MAX_TIME_MS });
  }
};
...
export const assertAvailabilityPreDropCheckpoint = (plan) => {
  if (!plan.safeToApply) throw new Error("Checkpoint pre-drop rechazado: el estado dejó de ser seguro");
...
export const runAvailabilityTenantization = async ({
  ...
  const validatedOptions = validateAvailabilityTenantizationOptions({ ...options }, mongoUri, processEnvironment);
  ...
  try {
    ...
    const initialPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    ...
    const lockedPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    if (!lockedPlan.safeToApply) throw new Error("Apply rechazado: el estado cambió antes del backfill");
    ...
    await runBackfillTransaction(connection, async (session) => {
      const transactionPlan = buildAvailabilityTenantizationPlan(
        await readAvailabilityTenantizationSnapshot(connection.db, { session }),
      );
      if (!transactionPlan.safeToApply) {
        throw new Error("Apply rechazado: el snapshot transaccional dejó de ser seguro");
      }
      // Las asignaciones autorizadas son las observadas bajo el lock antes de abrir
      // la transacción. Cada una se revalida de nuevo dentro de la transacción; si
      // Membership cambió, applyBackfill aborta en vez de adoptar silenciosamente
      // una inferencia distinta.
    ...
    const postBackfillPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    if (
    ...
    for (const [collectionName, spec] of [
      ...
      await checkpointLock();
      await stageCheckpoint(`before-create-index:${collectionName}`, { db: connection.db, lock });
      await checkpointLock();
    ...
    await checkpointLock();
    const preDropPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    assertAvailabilityPreDropCheckpoint(preDropPlan);

    await stageCheckpoint("before-drop-indexes", { db: connection.db, lock });
    await checkpointLock();
    const finalPreDropPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    ...
    await checkpointLock();
    const finalPlan = buildAvailabilityTenantizationPlan(await readAvailabilityTenantizationSnapshot(connection.db));
    ...
    if (lock && connected) {
      const released = await releaseLock(connection.db, lock).catch(() => false);
      if (!released) {
        // Nunca se elimina un lock ajeno. Si se perdió ownership, la ejecución ya debe considerarse fallida.
      }
    }
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  const result = await runAvailabilityTenantization({ mongoUri: process.env.MONGO_URI, options });
...
if (isDirectExecution) {
  main()
    ...
    .catch((error) => {
      console.error(`Migración 6.2.3 rechazada: ${sanitizeAuditErrorMessage(error, process.env.MONGO_URI)}`);
      process.exitCode = 1;
    });
}
